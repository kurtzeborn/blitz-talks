...
app.http('listGamekeepers', {
  ...
  handler: async (request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    try {
      ...
      context.error('Failed to list gamekeepers:', error);
      return { status: 500, jsonBody: { error: 'Failed to list gamekeepers' } };
    }
  },
});
    ...
    try {
      ...
      try {
        ...
        if (error.statusCode === 409) {
          return { status: 409, jsonBody: { error: 'This email is already a gamekeeper' } };
        }
        throw error;
      }
      ...
      context.error('Failed to invite gamekeeper:', error);
      return { status: 500, jsonBody: { error: 'Failed to invite gamekeeper' } };
    }
  },
});
    ...
    try {
      ...
      try {
        ...
        if (error.statusCode === 404) {
          return { status: 404, jsonBody: { error: 'Gamekeeper not found' } };
        }
        throw error;
      }
      ...
      context.error('Failed to remove gamekeeper:', error);
      return { status: 500, jsonBody: { error: 'Failed to remove gamekeeper' } };
    }
  },
});
...
app.http('seedGamekeeper', {
  ...
  handler: async (_request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> => {
    try {
      ...
      try {
        await gamekeepersTable.createEntity(entity);
        return { status: 201, jsonBody: { message: 'Seeded initial gamekeeper' } };
      } catch (error: any) {
        if (error.statusCode === 409) {
          return { status: 200, jsonBody: { message: 'Initial gamekeeper already exists' } };
        }
        throw error;
      }
    } catch (error) {
      context.error('Failed to seed gamekeeper:', error);
      return { status: 500, jsonBody: { error: 'Failed to seed gamekeeper' } };
    }
  },
});
